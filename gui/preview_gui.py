...
class GifPreviewWindow:
    ...
    def __init__(self, parent, frames, duration, output_path):
        ...
        if self.frames:
            ...
            # 如果图片原始尺寸过大，则按比例缩�?            if img_width > max_width or img_height > max_height:
            ...
            if window_width / window_height > 16 / 9:
                # 宽度过大，调整宽�?                window_width = int(window_height * 16 / 9)
            else:
                # 高度过大，调整高�?                window_height = int(window_width * 9 / 16)
    ...
    def setup_ui(self):
        """设置UI界面"""
        # 主框�?        main_frame = ttk.Frame(self.window, padding="10")
        main_frame.pack(fill=tk.BOTH, expand=True)

        # 配置主框架的网格权重，让图片显示区域获得大部分垂直空�?        main_frame.rowconfigure(0, weight=1)  # 图片显示区域
        ...
        # 创建一个居中容�?        center_container1 = ttk.Frame(control_frame1)
        ...
        # 进度�?        self.progress_var = tk.DoubleVar(value=0)
        ...
        # 当前帧显�?        self.frame_label = ttk.Label(center_container1, text="0 / 0", width=10)
    ...
    def on_mousewheel(self, event):
        ...
        if ctrl_pressed:
            # Ctrl+滚轮：缩放图�?            if event.delta > 0 or event.num == 4:
                ...
                # 向下滚动：缩�?                self.zoom_out()
        else:
            # 普通滚轮：滚动查看
            # 检查滚动区域是否大于Canvas可视区域，如果是则允许滚�?            bbox = self.canvas.bbox("all")
            if bbox:
                canvas_width = self.canvas.winfo_width()
                canvas_height = self.canvas.winfo_height()

                # 如果图片的宽度或高度大于Canvas的可视区域，则允许滚�?                if bbox[2] > canvas_width or bbox[3] > canvas_height:
                    # 检查操作系统类型来确定滚动方向
    ...
    def display_frame(self, frame_index):
        ...
        if 0 <= frame_index < len(self.frames):
            ...
            if cache_key in self.photo_cache:
                ...
                # 调整图片大小，根据缩放方向选择合适的插值算�?                frame_copy = frame.copy()
                if scale >= 1.0:
                    # 放大时使用高质量插值，保持清晰�?                    resampling = Image.Resampling.LANCZOS
                else:
                    # 缩小时使用双线性插值，提高性能
                    resampling = Image.Resampling.BILINEAR
                frame_copy = frame_copy.resize((display_width, display_height), resampling)

                # 转换为PhotoImage并缓�?                self.photo = ImageTk.PhotoImage(frame_copy)
    ...
    def zoom_in(self):
        """放大画面"""
        # 检查放大后是否会超出边�?        if self.zoom_scale < 10.0:  # 设置最大缩放倍数
