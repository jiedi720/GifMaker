...
文件大小: {size_kb:.2f} KB"""
        ...
        messagebox.showinfo("图片属�?, info_text)
    except Exception as e:
        messagebox.showerror("错误", f"无法读取图片属�? {str(e)}")
...
def copy_images(main_window_instance, index):
    ...
    # 将当前选中的图片索引添加到剪贴�?    main_window_instance.clipboard_images = [index]
    main_window_instance.clipboard_action = 'copy'
    print(f"已复制图�?#{index + 1}")
...
def paste_images(main_window_instance, target_index):
    ...
    try:
        # 保存当前状�?        from function.history_manager import save_state as save_main_window_state
        ...
        # 清空剪贴�?        main_window_instance.clipboard_images = []
...
def delete_images(main_window_instance, index):
    ...
    result = messagebox.askyesno("确认删除", f"确定要删除图�?\n{filename}?")
